...
func TestHashIndex_Find_Vs_Modify_Metrics(t *testing.T) {
	...
	for _, rCount := range recordCounts {
		...
		t.Logf("[%d recs] Hash Find:sc.Avail=%.4f,sc.Mean=%.6fs, P99=%.6fs (Buckets: %d)", rCount, findAvail, findMean, findP99, findOutcomes.Len())
		...
		t.Logf("[%d recs] Hash Insert:sc.Avail=%.4f,sc.Mean=%.6fs, P99=%.6fs (Buckets: %d)", rCount, insertAvail, insertMean, insertP99, insertOutcomes.Len())
		...
		t.Logf("[%d recs] Hash Delete:sc.Avail=%.4f,sc.Mean=%.6fs, P99=%.6fs (Buckets: %d)", rCount, deleteAvail, deleteMean, deleteP99, deleteOutcomes.Len())

		// --- Compare Metrics ---
		if findMean >= deleteMean {
			t.Errorf("[%d recs] Hash Findsc.Mean Latency (%.6fs) should typically be less than Deletesc.Mean (%.6fs)", rCount, findMean, deleteMean)
		}
