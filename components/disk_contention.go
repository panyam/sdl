...
func (d *DiskWithContention) Init() {
	...
	d.baseDisk = NewDisk(d.ProfileName)
	...
	case ProfileHDD:
		...
		d.queue = &MM1Queue{
			Name:           fmt.Sprintf("%s-queue", d.ProfileName),
	...
	default:
		...
		d.pool = &ResourcePool{
			Name:        fmt.Sprintf("%s-pool", d.ProfileName),
...
func NewDiskWithContention(profileName string) *DiskWithContention {
	d := &DiskWithContention{ProfileName: profileName}
	d.Init()
	return d
}
