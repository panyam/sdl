...
func TestCache_Read_Metrics(t *testing.T) {
	...
	hits, missesAndFailures := outcomes.Split(sc.AccessResult.IsSuccess)
	...
	if hits != nil && hits.Len() > 0 {
		meanHitLat := sc.MeanLatency(hits) // MeanLatency works as Success=true here means Hit
		expHitLat := Nanos(100)
...
func TestCache_Write_Metrics(t *testing.T) {
	...
	avail := sc.Availability(outcomes) // Success = successful write to cache
