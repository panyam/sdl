...
type NetworkLink struct {
	...
	transferOutcomes *Outcomes[AccessResult]
...
func (nl *NetworkLink) calculateTransferOutcomes() {
	...
	if successProb > 1e-9 { // Only add success buckets if probability is non-zero
		...
		for i := 0; i < nl.LatencyBuckets; i++ {
			...
			outcomes.Add(bucketWeight, AccessResult{
				Success: true,
				Latency: latency,
			})
		}
	...
	if nl.PacketLossProb > 1e-9 {
		...
		outcomes.Add(nl.PacketLossProb, AccessResult{
			Success: false,
...
func (nl *NetworkLink) Transfer() *Outcomes[AccessResult] {
