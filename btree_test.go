...
func TestBTreeIndex_Operations_Metrics(t *testing.T) {
	...
	singleReadMean := MeanLatency(bt.Disk.Read())
	...
	expectedInsertDeleteAvail := Availability(findOutcomes) * Availability(bt.Disk.Write()) * Availability(And(bt.Disk.Read(), bt.Disk.Write(), AndAccessResults))
