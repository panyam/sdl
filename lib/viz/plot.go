...
func (p *SVGPlotter) Generate(series []DataSeries, title, xLabel, yLabel string) (string, error) {
	...
	xTimeExtent, xFloatExtent, yExtent := p.findMultiSeriesExtents(data.Series)
	finalYExtent := p.adjustValueExtent(yExtent, p.config.YAxisMode)

	var xTicks []XTick
	var yTicks []YTick
	var seriesPaths []SeriesPath
	var bars []Bar

	yScale := p.createLinearScale(finalYExtent, innerHeight)
	yTicks = p.generateYTicks(yScale)

	if p.config.PlotType == LineChart {
		xScale := p.createTimeScale(xTimeExtent, innerWidth)
		xTicks = p.generateTimeXTicks(xScale, innerHeight)
		for i, s := range data.Series {
			color := p.config.Colors[i%len(p.config.Colors)]
			seriesPaths = append(seriesPaths, SeriesPath{
				Path:  p.generateLinePath(s.Points, xScale, yScale),
				Color: color,
			})
		}
	} else if p.config.PlotType == BarChart {
		// For bar charts, pad the x-axis so bars aren't cut off at edges
		xPadding := (xFloatExtent[1] - xFloatExtent[0]) * 0.05
		xScale := p.createLinearScale([2]float64{xFloatExtent[0] - xPadding, xFloatExtent[1] + xPadding}, innerWidth)
		xTicks = p.generateLinearXTicks(xScale, innerHeight)

		// Calculate bar width based on the minimum distance between points
		minDist := math.Inf(1)
		if len(series[0].Points) > 1 {
			for i := 1; i < len(series[0].Points); i++ {
				dist := float64(series[0].Points[i].X - series[0].Points[i-1].X)
				if dist < minDist {
					minDist = dist
				}
			}
		...
		if math.IsInf(minDist, 1) || minDist == 0 {
			// Fallback if only one point or points have same X
			minDist = (xFloatExtent[1] - xFloatExtent[0]) / 2.0
			if minDist == 0 {
				minDist = 1
			}
		}

		barWidth := float64(xScale.scale(xFloatExtent[0]+minDist)-xScale.scale(xFloatExtent[0])) * 0.8

		for i, s := range data.Series {
			color := p.config.Colors[i%len(p.config.Colors)]
			for _, pt := range s.Points {
				xPos := xScale.scale(float64(pt.X))
				yPos := yScale.scale(pt.Y)
				bars = append(bars, Bar{
					X:      float64(xPos) - barWidth/2,
					Y:      float64(yPos),
					Width:  barWidth,
					Height: float64(innerHeight) - float64(yPos),
					Color:  color,
				})
			}
		}
...
func (p *SVGPlotter) findMultiSeriesExtents(series []DataSeries) (xTime [2]time.Time, xFloat [2]float64, y [2]float64) {
	y = [2]float64{math.Inf(1), math.Inf(-1)}
	xFloat = [2]float64{math.Inf(1), math.Inf(-1)}
	xSet := false
	...
	for _, s := range series {
		...
		if !xSet {
			xTime[0] = time.Unix(0, s.Points[0].X*int64(time.Millisecond))
			xTime[1] = xTime[0]
			xFloat[0], xFloat[1] = float64(s.Points[0].X), float64(s.Points[0].X)
			xSet = true
		}
		for _, pt := range s.Points {
			...
			if float64(pt.X) < xFloat[0] {
				xFloat[0] = float64(pt.X)
			}
			if float64(pt.X) > xFloat[1] {
				xFloat[1] = float64(pt.X)
			}
			if pt.Y < y[0] {
				y[0] = pt.Y
			}
	...
	if y[0] > y[1] { // Handle empty or single-point data
		y[0], y[1] = 0, 100
	}
	if xFloat[0] > xFloat[1] {
		xFloat[0], xFloat[1] = 0, 100
	}
	if !xSet {
		xTime[0], xTime[1] = time.Now(), time.Now()
	}

	return xTime, xFloat, y
}
...
func (p *SVGPlotter) createLinearScale(extent [2]float64, length int) linearScale {
	return linearScale{domain: extent, rangeY: [2]int{length, 0}}
}
func (ts timeScale) scale(t time.Time) int {
	d := ts.domain[1].Sub(ts.domain[0])
	if d == 0 {
		return ts.rangeX[0]
	}
...
func (ls linearScale) scale(v float64) int {
	d := ls.domain[1] - ls.domain[0]
	if d == 0 {
		return ls.rangeY[0]
	}
	r := (v - ls.domain[0]) / d
	// Note: For Y axis, range is typically [height, 0], for X axis it's [0, width]
	// This single function works for both if rangeY is set correctly.
...
func (p *SVGPlotter) generateTimeXTicks(xs timeScale, h int) []XTick {
	var ticks []XTick
	numTicks := 8
...
func (p *SVGPlotter) generateLinearXTicks(xs linearScale, h int) []XTick {
	var ticks []XTick
	numTicks := 8
	valTicks := p.generateValueTicks(xs.domain[0], xs.domain[1], numTicks)
	prec := p.calculateOptimalPrecision(valTicks)
	for _, tick := range valTicks {
		ticks = append(ticks, XTick{X: xs.scale(tick), Label: p.formatValue(tick, prec)})
	}
	return ticks
}
...
func (p *SVGPlotter) generateGridLines(ys linearScale, w, h int) []GridLine {
	...
	for _, tick := range p.generateValueTicks(ys.domain[0], ys.domain[1], 6) {
		y := ys.scale(tick)
		lines = append(lines, GridLine{0, y, w, y})
	}
	return lines
}
...
func (p *SVGPlotter) generateValueTicks(min, max float64, maxTicks int) []float64 {
	...
	var step float64
	if normalizedStep := rawStep / magnitude; normalizedStep <= 1 {
		step = magnitude
...
func (p *SVGPlotter) calculateOptimalPrecision(values []float64) int {
	...
	if minDiff > 0 && !math.IsInf(minDiff, 0) {
		precision := int(math.Max(0, -math.Floor(math.Log10(minDiff)))) + 1
		if precision > 8 {
			return 8
		}
		return precision
