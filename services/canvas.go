...
func (c *Canvas) GetSystemDiagram() (*SystemDiagram, error) {
	...
	return BuildSystemDiagram(c.activeSystem, c.generators, c.currentFlowScope, c.GetCurrentFlowRates())
}
...
// SimulationContext interface implementation

// GetTracer returns the metric tracer for this canvas.
func (c *Canvas) GetTracer() runtime.Tracer {
	return c.metricTracer
}

// GetSimulationStartTime returns when the simulation was started.
func (c *Canvas) GetSimulationStartTime() time.Time {
	return c.simulationStartTime
}

// IsSimulationStarted returns true if the simulation clock is running.
func (c *Canvas) IsSimulationStarted() bool {
	return c.simulationStarted
}
