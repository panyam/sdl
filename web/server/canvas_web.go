...
func (ws *WebServer) handleFilesystemsAPI(w http.ResponseWriter, r *http.Request) {
	...
	if canvas.GetRuntime() == nil || canvas.GetRuntime().Loader == nil {
		http.Error(w, "Canvas runtime not initialized", http.StatusInternalServerError)
		return
	}

	// Get filesystem information
	filesystems := ws.getFileSystemInfo(canvas.GetRuntime().Loader)

	// Return JSON response
	w.Header().Set("Content-Type", "application/json")
	if err := json.NewEncoder(w).Encode(filesystems); err != nil {
		log.Printf("Failed to encode filesystems response: %v", err)
		http.Error(w, "Failed to encode response", http.StatusInternalServerError)
	}
}

// getDefaultCanvas retrieves the default canvas
func (ws *WebServer) getDefaultCanvas() *services.Canvas {
	if ws.canvasService == nil {
		return nil
	}

	return ws.canvasService.GetDefaultCanvas()
}
