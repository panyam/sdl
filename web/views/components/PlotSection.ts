...
import { SectionContent, PlotContent, SectionData, SectionCallbacks } from './types'; // Or move interfaces
    ...
    protected resizeContentForFullscreen(isEntering: boolean): void {
     ...
     protected override updateInternalContent(newContent: SectionContent): void {
         if (newContent && typeof newContent === 'object' && 'format' in newContent && 'data' in newContent) {
             this.plotContent = newContent as PlotContent;
             this.plotConfig = this.plotContent.data || {}; // Update plotConfig as well
         } else {
              console.warn(`PlotSection ${this.data.id}: Received invalid content during updateInternalContent`, newContent);
         }
     }
}
