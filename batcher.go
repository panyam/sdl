...
type BatchProcessor interface {
	ProcessBatch(batchSize uint) *Outcomes[AccessResult]
}
...
func (b *Batcher) Submit() *Outcomes[AccessResult] {
	...
	// Map wait time Outcomes[Duration] to Outcomes[AccessResult] (assuming waiting itself succeeds)
	...
	if downstreamOutcomes == nil {
		return (&Outcomes[AccessResult]{}).Add(1.0, AccessResult{Success: false, Latency: 0})
	}

	// 3. Combine Waiting Time + Downstream Processing Time
	finalOutcomes := And(waitAccessOutcomes, downstreamOutcomes, func(waitRes, downstreamRes AccessResult) AccessResult {
		return AccessResult{Success: downstreamRes.Success, Latency: waitRes.Latency + downstreamRes.Latency}
	})
