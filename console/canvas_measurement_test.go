...
func TestCanvas_EndToEnd_MeasurementWorkflow(t *testing.T) {
	...
	err = canvas.AddCanvasMeasurement("lat1", "Server Latency", "server.HandleLookup", "latency", true)
	...
	err = canvas.Run("baseline", "server.HandleLookup", WithRuns(10))
	...
	metrics, err := canvas.GetMeasurementMetrics("server.HandleLookup", since)
	...
	p50, p90, p95, p99, err := canvas.GetMeasurementPercentiles("server.HandleLookup", since)
	...
	err = canvas.Run("load_test", "server.HandleLookup", WithRuns(50))
	...
	sqlResult, err := canvas.ExecuteMeasurementSQL("SELECT COUNT(*) as count FROM traces WHERE target = ?", "server.HandleLookup")
	...
	err = canvas.Run("no_measurements", "server.HandleLookup", WithRuns(25))
