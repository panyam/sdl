...
func TestDisk_PerformanceMetrics(t *testing.T) {
	...
	ssdReadAvail := Availability(ssd.Read())
	ssdReadMean := MeanLatency(ssd.Read())
	ssdReadP99 := PercentileLatency(ssd.Read(), 0.99) // P99 of read latency
	ssdWriteMean := MeanLatency(ssd.Write())

	hddReadAvail := Availability(hdd.Read())
	hddReadMean := MeanLatency(hdd.Read())
	hddReadP99 := PercentileLatency(hdd.Read(), 0.99) // P99 of read latency
	hddWriteMean := MeanLatency(hdd.Write())
...
func TestDisk_ReadProcessWrite(t *testing.T) {
	...
	rpwAvail := Availability(rpwOutcomes)
	rpwMean := MeanLatency(rpwOutcomes)

	// Expected availability = read_avail * write_avail
	expectedAvail := Availability(ssd.Read()) * Availability(ssd.Write())
	// Expected mean latency = read_mean + write_mean + processing_time (approximation)
	expectedMean := MeanLatency(ssd.Read()) + MeanLatency(ssd.Write()) + processingTime
