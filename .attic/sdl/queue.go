...
func (q *Queue) Enqueue() *Outcomes[AccessResult] {
	...
	if acceptProb > 1e-9 {
		outcomes.Add(acceptProb, AccessResult{Success: true, Latency: enqueueLatency})
	}
	if blockingProb > 1e-9 {
		outcomes.Add(blockingProb, AccessResult{Success: false, Latency: enqueueLatency}) // Blocked is immediate failure
	}
	if outcomes.Len() == 0 { // Handle case where probabilities might be exactly 0 or 1
		if acceptProb >= 0.5 {
			...
			outcomes.Add(1.0, AccessResult{Success: false, Latency: enqueueLatency})
		}
	}
