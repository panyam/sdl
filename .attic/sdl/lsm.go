...
func (lsm *LSMTree) Write() *Outcomes[AccessResult] {

	// 1. Base cost: Memtable update (CPU) + WAL write (Disk write)
	// We approximate this as RecordProcessingTime + one Disk Write.
	baseWriteCost := And(
		Map(&lsm.RecordProcessingTime, func(p Duration) AccessResult { return AccessResult{true, p} }), // Map processing time to AccessResult
		lsm.Disk.Write(), // Assumes WAL write has similar profile to general disk write
		AndAccessResults, // Combine CPU + Disk Write
	)
	...
	amplifiedWriteCost := Map(baseWriteCost, func(ar AccessResult) AccessResult {
		if ar.Success {
	...
	finalOutcomes := &Outcomes[AccessResult]{And: amplifiedWriteCost.And} // Use AndAccessResults
	...
	// Use the standard TrimToSize defined for AccessResult
	// Need to split -> trim -> append
	successes, failures := finalOutcomes.Split(AccessResult.IsSuccess)
	trimmer := TrimToSize(100, lsm.MaxOutcomeLen) // Use MaxOutcomeLen from Index
	trimmedSuccesses := trimmer(successes)
	trimmedFailures := trimmer(failures)
	finalTrimmedOutcomes := (&Outcomes[AccessResult]{}).Append(trimmedSuccesses, trimmedFailures)

	return finalTrimmedOutcomes
}

// Read simulates finding a key in the LSM tree.
func (lsm *LSMTree) Read() *Outcomes[AccessResult] {
	// --- Define costs for different read steps ---

	// Cost of checking Memtable (pure CPU)
	memtableCheckCost := Map(&lsm.RecordProcessingTime, func(p Duration) AccessResult {
	...
	l0CheckCost := And(
		Map(&lsm.RecordProcessingTime, func(p Duration) AccessResult { return AccessResult{true, p} }),
	...
	deepLevelBaseCheckCost := And(
		Map(&lsm.RecordProcessingTime, func(p Duration) AccessResult { return AccessResult{true, p} }),
		lsm.Disk.Read(),
		AndAccessResults,
	)
	// Amplify the latency part for successes
	deepLevelCheckCost := Map(deepLevelBaseCheckCost, func(ar AccessResult) AccessResult {
		if ar.Success {
	...
	memtableHits := Map(currentOutcomes, func(ar AccessResult) AccessResult { return ar }) // Keep as is
	...
	l0CheckPath := And(currentOutcomes, l0CheckCost, AndAccessResults) // Cost if we check L0

	// Path 2a: Hit L0 (Cost = Memtable Check + L0 Check) -> Final Weight = memtableMissProb * Level0HitProb
	l0Hits := Map(l0CheckPath, func(ar AccessResult) AccessResult { return ar })
	...
	deepCheckPath := And(l0CheckPath, deepLevelCheckCost, AndAccessResults) // Cost if we check deep

	// Path 3: Hit Deep Levels (Cost = Memtable Check + L0 Check + Deep Check) -> Final Weight = l0MissProb
	deepHits := Map(deepCheckPath, func(ar AccessResult) AccessResult { return ar })
	deepHits.ScaleWeights(l0MissProb)

	// Combine all paths
	combinedReadOutcomes := (&Outcomes[AccessResult]{}).Append(memtableHits, l0Hits, deepHits)

	// Factor in Compaction Interference (same logic as in Write)
	finalOutcomes := &Outcomes[AccessResult]{And: combinedReadOutcomes.And}
