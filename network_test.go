...
func TestNetworkLink_Transfer_Metrics_NoLoss(t *testing.T) {
	...
	p0 := PercentileLatency(outcomes, 0.0)   // Should be near Base - Jitter
	p50 := PercentileLatency(outcomes, 0.5)  // Should be near Base
	p100 := PercentileLatency(outcomes, 1.0) // Should be near Base + Jitter

	t.Logf("NoLoss Link: Avail=%.4f, Mean=%.6fs, P0=%.6fs, P50=%.6fs, P100=%.6fs", avail, meanLat, p0, p50, p100)

	// Assertions
	if !approxEqualTest(avail, 1.0, 1e-9) {
		t.Errorf("Availability mismatch: expected 1.0, got %.4f", avail)
	}
	// Mean should be close to BaseLatency for symmetric jitter distribution
...
func TestNetworkLink_Transfer_Metrics_WithLoss(t *testing.T) {
	...
	// Calculate metrics
	avail := Availability(outcomes)
	meanSuccLat := MeanLatency(outcomes) // Mean latency *of successful transfers*

	t.Logf("WithLoss Link: Avail=%.4f, MeanSuccessLatency=%.6fs", avail, meanSuccLat)
	...
	// Mean success latency should still be around BaseLatency
	...
	_, failures := outcomes.Split(AccessResult.IsSuccess)
